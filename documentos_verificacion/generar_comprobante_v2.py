...
def create_html_content(tx_id, client_name, date, amount, plan, platform):
    ...
    if "avanzado" in plan.lower():
    ...
    elif "integral" in plan.lower() or "semestral" in plan.lower():
        desc = "Auditoría de seguridad integral, remoción de malware, configuración de copias de seguridad automatizadas y asistencia técnica remota bajo demanda."
        ...
        desc = "Mantenimiento preventivo mensual para sistemas, optimización lógica de rendimiento y diagnóstico básico remoto."
    else:
...
<body>
    ...
    <div class="provider-info">
        ...
        <strong>Contacto:</strong> soporte@dantro.tech
    </div>
    ...
    <table class="details-table">
        <tr>
            <th>ID de Transacción {platform}</th>
            <td>{tx_id}</td>
        </tr>
        <tr>
            <th>Cliente</th>
...
def main():
    parser = argparse.ArgumentParser(description="Generar PDF de comprobante de entrega para PayPal/dLocalGo/Whop")
    ...
    parser.add_argument("--plataforma", required=True, help="Nombre de la plataforma (ej: Whop, dLocalGo)")
    ...
    html_content = create_html_content(args.id, args.cliente, args.fecha, args.monto, args.plan, args.plataforma)
    ...
    # Compile HTML to PDF
    # We must use file:/// protocol to avoid SmartScreen/Headless multiple target errors
    file_url = "file:///" + html_path.replace("\\", "/")
    print(f"Generando PDF {pdf_filename} desde {file_url}...")
    
    # Writing to a temp location first because of controlled folder access
    temp_dir = os.environ.get("TEMP", base_dir)
    ...
    # Copy file back from temp folder
